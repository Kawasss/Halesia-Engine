...
protected:
	...
	void BuildAS(const VulkanCreationObject& creationObject, const VkAccelerationStructureGeometryKHR* pGeometry, uint32_t primitiveCount, VkBuildAccelerationStructureModeKHR mode, bool useSingleTimeCommands = true, VkCommandBuffer externalCommandBuffer = VK_NULL_HANDLE);
...
public:
	...
	void Update(const VulkanCreationObject& creationObject, std::vector<Object*>& objects, VkCommandBuffer externalCommandBuffer);
	bool HasBeenBuilt();

private:
	static std::vector<VkAccelerationStructureInstanceKHR> GetInstances(std::vector<Object*>& objects);
	void GetGeometry(const VulkanCreationObject& creationObject, VkAccelerationStructureGeometryKHR& geometry);
