...
void ShaderCompiler::CallCompiler(const fs::path& file)
{
	...
	std::string args = std::format("-i {} -o shaders/spirv/{}.spv -Dbindless_texture_size={} -Dmaterial_buffer_binding={} -Dlight_buffer_binding={} -DDECLARE_EXTERNAL_SET(index) --target-env=vulkan1.4", file.string(), file.filename().string(), Renderer::MAX_BINDLESS_TEXTURES, Renderer::MATERIAL_BUFFER_BINDING, Renderer::LIGHT_BUFFER_BINDING); // can compile with -O for optimisations
