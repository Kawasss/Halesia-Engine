...
void Swapchain::CreateDepthBuffers()
{
    ...
    Vulkan::CreateImage(logicalDevice, physicalDevice, extent.width, extent.height, 1, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);
    depthImageView = Vulkan::CreateImageView(logicalDevice, depthImage, 1, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT);
}
...
void Swapchain::CreateImageViews()
{
    ...
    for (uint32_t i = 0; i < images.size(); i++)
        imageViews[i] = Vulkan::CreateImageView(logicalDevice, images[i], 1, format, VK_IMAGE_ASPECT_COLOR_BIT);
}
